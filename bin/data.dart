...
List<ViewModuleDto> viewModules(String mallType) {
  return [
    ...
    ViewModuleDto(type: 'banner_view_module', imageUrl: bannerImageList[Random().nextInt(5)]),
    ViewModuleDto(
...
List<ProductInfoDto> productGenerator(int length, String storeType) {
  ...
  var productList = storeType == 'market' ? marketProductList : beautyProductList;
  ...
  List<ProductInfoDto> list = List.generate(length, (index) {
    ...
    return productList[randomSet.elementAt(index)].copyWith(
      productId: '$originalPrice'.padLeft(6, '0') + '$discountRate'.padLeft(6, '0'),
